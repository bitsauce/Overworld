...
class TileGrid
{
		...
		for(int i = 0; i < textures.size; i++)
		{
			...
			for(int y = 0; y < height; y++)
			{
				for(int x = 0; x < width; x++)
				{
					for(int j = 0; j < 13; j++)
					{
						Sprite @sprite = @Sprite(@getTextureRegion(i+1, j));
	...
	void removeTile(const int x, const int y)
	{
		...
		int i = (y*width + x) * 13;
		for(int j = 0; j < 13; j++) {
			batches[tile-1].get(i+j).setColor(Vector4(0.0f));
		}
	...
	{
		return @TextureRegion(@tileTextures[tile-1], TILE_TEXTURE_COORDS[0, i], TILE_TEXTURE_COORDS[1, i], TILE_TEXTURE_COORDS[2, i], TILE_TEXTURE_COORDS[3, i]);
	}
	...
	void updateTile(const int x, const int y)
	{
		...
		if(tile != NULL_TILE)
		{
			...
			SpriteBatch @batch = batches[tile-1];
			...
			if(state & NORTH_WEST == 0)
			{
				...
				// Show outer corner
				if(state & NORTH == 0 && state & WEST == 0) {
					batch.get(i+0).setColor(Vector4(1.0f));
				}
			}else{
			...
			if(state & NORTH_EAST == 0)
			{
				...
				// Show outer corner
				if(state & NORTH == 0 && state & EAST == 0) {
					batches[tile-1].get(i+3).setColor(Vector4(1.0f));
				}
			}else{
			...
			{
				// Show outer corner
				if(state & SOUTH == 0 && state & EAST == 0) {
					batch.get(i+6).setColor(Vector4(1.0f));
				}
			}else{
				// Hide outer corner
			...
			{
				// Show outer corner
				if(state & SOUTH == 0 && state & WEST == 0) {
					batch.get(i+9).setColor(Vector4(1.0f));
				}
			}else{
				// Hide outer corner
